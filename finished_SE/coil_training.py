...
from transformers import TransformerEncoder, TransformerDecoder, DiagonalGaussian
from CoilAutoEncoder import CoilAutoencoderModel

# %%
tfrecord_dir = Path("modes_surface_coil_tfrecords")
...
class SaveBestSubmodels(tf.keras.callbacks.Callback):
    ...
    def __init__(self, autoencoder, encoder, decoder, log_dir, monitor="val_loss", mode="min"):
        super().__init__()
        self.autoencoder = autoencoder      # <-- CoilAutoencoderModel (has latent_head) :contentReference[oaicite:10]{index=10}
    ...
    def _build_q_encoder(self):
        ...
        # Use the *trained* latent_head from CoilAutoencoderModel :contentReference[oaicite:11]{index=11}
...
saved_encoder = tf.keras.models.load_model(
    "coil_logs/20260301-045816_trial/best/q_encoder.best.keras",
...
# %%
surface_data.numpy()[0]

# %%
coil_latents['mu'].numpy()[0, 0]
...
# %%
def write_latent_augmented_tfrecord(dataset, encoder, output_dir, 
                                    max_records_per_file=10000, latent_key='mu'):
    ...
    for batch, _ in tqdm(dataset, desc="Encoding & Writing"):
        ...
        if isinstance(out, dict):
            coil_latents = out[latent_key]
        else:
            coil_latents = out

        if len(coil_latents.shape) == 3 and coil_latents.shape[1] == 1:
            coil_latents = tf.squeeze(coil_latents, axis=1)
        ...
        for i in range(coil_data.shape[0]):
            ...
            serialized = serialize_example(coil_data[i], coil_mask[i], surface_data[i], 
                                           surface_mask[i], coil_latents[i])
