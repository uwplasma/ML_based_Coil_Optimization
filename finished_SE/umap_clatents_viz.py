...
from transformers import SelfAttentionBlock, PoolingByMultiheadAttention, LearnedQueryDecoder, DiagonalGaussian
...
def load_encoder_decoder(models_dir: Path):
    enc = tf.keras.models.load_model(
        str(models_dir / "q_encoder.best.keras"),
        custom_objects=CUSTOM_OBJECTS,
        safe_mode=False,
        compile=False,
    )
    dec_path = models_dir / "decoder.best.keras"
    dec = None
    if dec_path.exists():
