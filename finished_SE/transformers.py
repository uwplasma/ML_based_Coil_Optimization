...
@tf.keras.utils.register_keras_serializable(package="my_layers")
class DiagonalGaussian(tf.keras.layers.Layer):
    """
    Maps a hidden vector h to a diagonal Gaussian q(z|h) with parameters (mu, logvar),
    and returns (z, mu, logvar). During training it samples; at eval it returns mu.
    """
    def __init__(self, latent_dim, min_logvar=-8.0, max_logvar=8.0, **kwargs):
        super().__init__(**kwargs)
        self.latent_dim = int(latent_dim)
        self.min_logvar = float(min_logvar)
        self.max_logvar = float(max_logvar)
        self.to_params = layers.Dense(2 * self.latent_dim, name="z_params")

    def call(self, h, training=False, sample=True):
        # h: (B, H) — can be the output of your encoder’s last layer
        params = self.to_params(h)                          # (B, 2*D)
        mu, logvar = tf.split(params, 2, axis=-1)          # (B, D), (B, D)
        logvar = tf.clip_by_value(logvar, self.min_logvar, self.max_logvar)

        if training and sample:
            eps = tf.random.normal(tf.shape(mu))
            z = mu + tf.exp(0.5 * logvar) * eps            # reparameterization
        else:
...
def TransformerEncoder(max_sets, features_per_set, name, embed_dim, num_heads, ff_dim, num_sab_blocks, 
    ...
    return tf.keras.Model(inputs={data_type: input, mask_type: mask}, outputs=pooled, name=model_name)


'''SURFACE PERCEIVER ENCODER'''
...
#     def call(self, latents, inputs, training=False, attn_mask=None):
...
#         lat = block(lat, x, attn_mask=attn_mask)  # (B, L, D)

#     # final normalization
#     lat = layers.LayerNormalization(epsilon=1e-6, name=f"{name}_final_norm")(lat)

#     PMA = PoolingByMultiheadAttention(embed_dim, num_heads, dropout=dropout)
#     pooled = PMA(lat) # (B, num_seeds, embed_dim)
...
class LearnedQueryDecoder(tf.keras.layers.Layer):
    ...
    def call(self, encoded_set, training=False):
        ...
        for i in range(self.num_layers):
            ...
            x = self.norm1_layers[i](x + self._film(attn_out, encoded_set, self.film_g1[i], self.film_b1[i]))

            # Feed-forward network
            ff_out = self.ffn_layers[i](x, training=training)
            x = self.norm2_layers[i](x + self._film(ff_out, encoded_set, self.film_g2[i], self.film_b2[i]))
...
def TransformerDecoder(name, embed_dim, num_heads, ff_dim, num_layers, 
    ...
    return tf.keras.Model(inputs=encoded_latent, outputs=output, name=decoder_name)