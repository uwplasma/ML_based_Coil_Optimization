...
def parse_tfrecord_fn(example_proto):
    ...
    'surface_data': tf.io.FixedLenFeature([TOTAL_SETS], tf.float32),
    'surface_mask': tf.io.FixedLenFeature([MAX_SETS], tf.int64),
    'coil_latent': tf.io.FixedLenFeature([EMB], tf.float32)
}
    parsed = tf.io.parse_single_example(example_proto, feature_description)
    
    coil_data = tf.reshape(parsed["coil_data"], [MAX_COILS, FEATURES_PER_COIL])
    coil_mask = parsed["coil_mask"]
    surface_data = parsed['surface_data'] #tf.reshape(parsed['surface_data'], [TOTAL_SETS, FEATURES_PER_SET])
    ...
    target = {'coil': coil_data, 'surface': surface_data, 'coil_latent': coil_latent}
